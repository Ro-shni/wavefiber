...
export const chatApi = {
  ...
  getMessages: async (complaintId: string): Promise<{ success: boolean; messages: ChatMessage[] }> => {
    const response = await api.get(`/chat/${complaintId}/messages`)
    return response.data
  },

  sendMessage: async (complaintId: string, content: string): Promise<{ success: boolean; message: ChatMessage }> => {
    const response = await api.post(`/chat/${complaintId}/messages`, { content })
    return response.data
  },

  sendVoiceMessage: async (complaintId: string, audioBlob: Blob, duration: number): Promise<{ success: boolean; message: ChatMessage }> => {
